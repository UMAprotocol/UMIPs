...
- "VERSION"
  - Across protocol version number. Supporting implementations should query this value against a locally stored value to determine compatibility with the current protocol implementation. Failure to correctly evaluate the version number may mean that filled relays are not refunded from the HubPool, and may therefore result in loss of relayer funds.
