...
| UMIP-85    |                                                          |
|------------|----------------------------------------------------------|
| UMIP Title | Add CADUSD and JPYUSD as price identifiers                           |
| Authors    | Pascal Tallarida (pascal@jarvis.network)                 |
| Status     | Draft                                                 |
| Created    | April 28,2021                                   |

## Summary
The DVM should support price requests for the CADUSD and JPYUSD price index.

## Motivation
The DVM currently does not support the CADUSD and JPYUSD index.
...
## Implementation
Historical CADUSD and JPYUSD prices from TraderMade are available on minute increments. Price requests should use the minute price that is nearest and earlier than the price request timestamp. To do this, voters should use the open price of the OHLC period that the price request timestamp falls in. TraderMade endpoints are queried based on the OHLC period's close time.
...
CADUSD, and JPYUSD should then be rounded to 5 decimals.
